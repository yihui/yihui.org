...
When a code chunk produces an HTML widget (e.g., from the **htmlwidgets**
package) or a Shiny app, and the output format is not HTML (e.g., LaTeX or Word),
**knitr** takes a static screenshot of it. The screenshot is generated by the
[**webshot2**](https://cran.r-project.org/package=webshot2) package if it is
installed (which requires a headless Chrome/Chromium browser), or the older
[**webshot**](https://cran.r-project.org/package=webshot) package (which
requires PhantomJS). **webshot2** is preferred automatically when both are
available, since PhantomJS has been unmaintained since 2018. The following chunk
options control the screenshotting behavior:

-   `webshot`: (`NULL`; character) Which package to use to take the screenshot,
    either `'webshot2'` or `'webshot'`. If unset, **knitr** will use **webshot2**
    when it is available, and fall back to **webshot** otherwise. You normally do
    not need to set this option; set it only to force a particular package, e.g.,
    `opts_chunk$set(webshot = 'webshot')` to keep using the old **webshot**
    package.

-   `screenshot.force`: (`NULL`; logical) Whether to force taking screenshots.
    If `TRUE`, screenshots are always taken even when the output format supports
    HTML (in which case the widget would normally be embedded live). If `FALSE`,
    screenshots are never taken, and the HTML content is rendered as-is.

-   `screenshot.opts`: (`NULL`; list) A list of options passed to
    `webshot2::webshot()` (or `webshot::webshot()`), e.g.,
    `screenshot.opts = list(vwidth = 6 * 72, vheight = 6 * 72 * 0.618)` to set
    the viewport width and height, or `list(delay = 3)` to wait longer before
    capturing.

-   `screenshot.alt`: (`NULL`; character) A character vector of paths to image
    files to be used as the screenshots, instead of having **knitr** generate
    them. When there are multiple screenshots in a chunk, the images are used in
    order.
...
-   `knitr.warn.webshot2`: (`TRUE`; logical) Whether to warn when **webshot2** is
    used to take a PDF screenshot (i.e., when the chunk option `dev` is `'pdf'`),
    because **webshot2** may use the wrong figure size for PDF. Set to `FALSE` to
